import { useDispatch, useSelector } from "react-redux";

import { potentialCountriesActions } from "../store/slices/potentialCountriesSlice";
import { displayCountryActions } from "../store/slices/displayCountrySlice";
import { selectDispaly } from "../store/slices/displayCountrySlice";

import axios from "axios";
import React, { useState } from "react";
import { BsFillFlagFill } from "react-icons/bs";

const Header = () => {
  const [input, setInput] = useState();

  const currentDisplay = useSelector(selectDispaly);

  const dispatch = useDispatch();

  return (
    <div className="header">
      <div className="home">
        <BsFillFlagFill style={{ marginRight: "10px" }} fontSize="1.6em" />
        <h3 className="home-country">
          {currentDisplay && currentDisplay.name.common}
        </h3>
      </div>
      <div className="country-input">
        <input
          onChange={(e) => {
            setInput(e.target.value);
          }}
        />
        <button
          onClick={() => {
            axios
              .get(`https://restcountries.com/v3.1/name/${input}`)
              .then((res) => {
                console.log(res.data);
                dispatch(potentialCountriesActions.deletePotentialCountries());
                dispatch(
                  potentialCountriesActions.setPotentialCountries(res.data)
                );
                dispatch(displayCountryActions.deleteDisplayCountry());
              })
              .catch((err) => {
                alert("No countries found that match your search!");
              });
          }}
        >
          search
        </button>
      </div>
    </div>
  );
};

export default Header;
